...
def main():
    ...
    while True:
        ...
        if status_input in statuses:
            ...
            filtered_transactions = [t for t in transactions if t.get("status", "").lower() == status_input]
            print(f"Найдено {len(filtered_transactions)} транзакций с статусом \"{status_input.upper()}\".")
            break
        else:
