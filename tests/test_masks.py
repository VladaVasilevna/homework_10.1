...
def valid_card_numbers():
    return [
        ("1234567812345678", "1234 56** **** 5678")  # Ожидается: "1234 56** **** 5678"
    ]
...
def test_masking_correct_number(card_number, expected):
...
def test_masking_correct_number(valid_card_numbers):
    """Тестирование правильности маскирования номера карты."""
    for card_number, expected in valid_card_numbers:
        assert get_mask_card_number(card_number) == expected

def test_masking_edge_cases(invalid_card_numbers):
...
def test_masking_correct_account(account_numbers):
...
def test_masking_edge_cases_accounts(account_number, expected):
